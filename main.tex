...
We have implemented a river routing scheme [RVIC] within the recently developed Regional Arctic System Model [RASM]\marginpar{needs reference; if there is no other paper to cite, use your own} to represent the streamflow flux between the land and ocean model components.
...
\citet{Newton_2008} applied observed climatological runoff at the largest nine rivers in the Arctic basin and used dye tracers \marginpar{I do not understand - ``dye'' tracers evokes the idea of physical tracers, while you appear to be talking about a model study here. Please clarify} to visualize the spatial distribution of runoff; finding the highest concentrations of river runoff along the Siberian coast.
...
Source-to-sink routing methods do not explicitly track streamflow between grid cells. Instead, they parameterize the distribution and travel time of runoff between source and outlet grid cells.\marginpar{Need to make clear how the approach here differs. As-is, it is a setup for reviewers to complain: ``What's new?''. So explain in a few lines how your approach differs}
...
\subsection{RASM}
\label{sec:rasm}\marginpar{Would be nice if we could shorten this, but without a solid RASM reference not sure that is possible. Leave as-is for now.}
...
\item VIC: The Variable Infiltration Capacity model \citep{Liang_1994}\marginpar{Please use Liang et al. 1994 as the primary VIC reference} is a macroscale land surface hydrology model.
...
Models are coupled every 20 minutes in the configuration described by \citet{Roberts_2015a}.\marginpar{Describe in one sentence why this coupling frequency is important - \citet{Roberts_2015a} makes that clear}
...
RVIC is a source-to-sink model that solves a linearized version of the Saint-Venant equations.\marginpar{needs a reference and a one-line description of what the Saint-Venant equations are}
...
\label{sec:irfs}\marginpar{You need a bit more of a lead in that explains all the pieces and how they fit together - as I am reading this section it is rather unclear why I am reading this. Why are we using IRFs rather than tracking the flow through the channel or using a reach-based routing model. You have to give some background before you launch into IRF and Green's functions. Be kind to your readers and tell a story}

The source-to-sink transfer of water from any model grid cell to a downstream (outlet) point is represented as a linear and time invariant response to an impulse of runoff.\marginpar{Very jargonny - try to reword this in a way that it makes sense for people who have not looked at unit hydrographs}
Therefore, the development of the IRF between any combination of source and sink points can be described as a function of the travel time of water within the source grid cell and to the downstream point with the addition of a flow diffusion parameter. \marginpar{Confusing - separate the ``within grid cell'' from the ``between grid cell'' transport}
...
Eq.~(\ref{eq:StVenant})\marginpar{Use references - that is one of the strengths of \latex - I am not sure I did this quite right in this case cause it starts at Equation 2, but it should be very close} can be solved with convolution integrals
...
\subsubsection{Upscaling and basin aggregation}\marginpar{As for the previous section this all makes perfect sense if you already know what is going on, but that is not the main audience. Be explicit about what is being preserved, etc.}
\label{sec:remap}

Whereas the typical employment of the \citet{Lohmann_1996} model requires that the flow direction grid and the land surface model grid match exactly, the RVIC implementation allows the development of the impulse response functions on a grid that is of higher resolution than that of the model by upscaling and aggregating the high resolution IRF grid to the land surface model grid [Figure \ref{fig:2}]\marginpar{It's fine as is, but in general you should not hardcode the fig numbers in latex, so you can move them around}.\marginpar{Again tell a bit more of a story: why is this desirable? What ``problem'' does this solve?}
The upscaling process uses the first-order conservative remapping technique developed by \citep{Jones_1999}.
Because the remapping scheme is conservative, the unit response to runoff (unit hydrograph) from each source grid cell is maintained.\marginpar{What does that mean in plain english - what is being conserved?}
Finally, the regridded IRFs are aggregated to include all basins flowing into a single outlet grid cell.

\subsubsection{Convolution}\marginpar{A little more detail would again be helpful - refer to Lohmann's papers when you can}
\label{sec:convolution}\marginpar{Refer to the convolution integral \ref{eq:ConvIntegral}}
...
The mapping from the runoff grid to the ocean grid is produced \marginpar{Not sure what ``produced'' means in this context} as a preprocessing step using the runoff and ocean grids and masks.
Each runoff gridcell is mapped to the nearest ocean grid cell.
The flux is then smoothed over all grid cells in a 300 km radius with distance weighting e-folding scale of 1000 km \marginpar{What is a "distance weighting e-folding scale''?} such that the total quantity is conserved from runoff to ocean.
This smoothing prevents excessively large and unrealistic runoff fluxes into any single ocean grid cell and encourages computational stability within the ocean model while representing the distributed changes in salinity due to the land-ocean freshwater flux.\marginpar{Work on the flow in this paragraph - you already said this}
...
The overlap statistic is perhaps the best performance measure of the routing model because it focuses entirely on the shape of the hydrograph and does not take into account bias in the flow volume which is determined by the land surface model [Figure \ref{fig:3}].\marginpar{The figure is not very informative - I would leave it out or move it to supplemental materials. We can describe the figure in words}
...
All three simulations were run from September 1, 1979 through December 31, 2009. However, we use an analysis period of January 1, 1990 and December 31, 2009, allowing for a 10 year model spin-up.\marginpar{Make sure that this is RASM spinup the routing model requires only very little spinup}
...
Compared to the normalized hydrographs in Figure \ref{fig:4}, the majority of the disagreement in Figure \ref{fig:5} is due to the bias in the total annual runoff flux, which is entirely controlled by the land surface model.\marginpar{not really - the rest of RASM comes into this as well. Just say that it is not under the control of RVIC which is a mass-conserving routing model}
...
On average, this leads to values for the overlap statistic \marginpar{it's one statistic (not ``statistics'' that can have different values} that are about 15\% lower than for the $RASM_{CONTROL}$ simulation.
...
Runoff is a major source of freshwater to the Arctic Ocean\marginpar{I think in much of the literature this is used for ``Arctic Oscillation'' - a climate fluctuation like ENSO - I would just write out Arctic Ocean}. It maintains the vertical stratification and may influence the ocean circulation.
...
We begin by exploring\marginpar{too chatty} the influence of coastal streamflow on the mean surface salinity and temperature across the RASM ocean domain.
