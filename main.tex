...
% BN Make clear how it as extended. From the seasonal or annual?
...
% BN If you are using the overlap statistic here in a different way (for example without normalizing), then I don't think it is necessarily the right statistic and an RMSE may be better.
...
% BN I find the difference between Fig 4 and 5 confusing. In one you use observations, while in the second you use D2009. Aren't they the same thing here?
...
% BN Again - I don't think the overlap statistic is the right statistic if you are not normalizing, because the maximum value in that case is not 1 (I think). If you are using the actual volumes, then I think an RMSE is probably better.
...
% BN rephrase the first part of the next sentence. I find it awkward.
...
% BN Rephrase next sentence (and maybe break it up). I don't know what you mean by the second part of this sentence.
...
% BN Doesn't D2009 provide estimates for the ungauged area? We need to make sure this is a  fair comparison.
...
% BN This is a bit confusing since we have no solid ice calving flux at all. So if you account for that, how do we compare?
...
The adjustment period in the $RASM_{CONTROL}$ simulation is largely driven by a change in the ocean forcings, from uncoupled using $CORE.v2$ to coupled within RASM. % BN How do you know that this is the main reason?
The change in salinity during the first ten years of the $RASM_{CONTROL}$ simulation is mainly driven by a change in the freshwater flux from streamflow but also includes the impact of different atmospheric forcings. % BN Where do the different atmospheric forcings come from?
By 2010, the SSS differs between the two simulations by about 0.6 ppt (parts per thousand) for the full ocean domain and by 1.5 ppt for the central arctic basin. % BN Can you put that number in context? Is that large? The audience is a mix of hydrologists and some oceanographers. The latter may have a sense about this number, but I do not.
...
As we discussed in Section \ref{sec:intro}, the earlier freeze-up resulting from surface freshening, as occurred in $RASM_{CONTROL}$, is related to net warming of the SSTs. % BN repetitive. Some cleanup and streamlining would be good.
This result partially corroborates the findings of \citep{Morison_2012} insofar as they also indicated, from an observational perspective, that a fresher Arctic Ocean would have less sea ice. % BN Can you clarify how this works? I would have expected that the fresher water would freeze up more easily.
...
% BN: Include a statement where the data can be obtained.

% BN: Don't promise things at the end of papers. Manuscripts should be standalone.
...
% BN right- or .-align the table. Also make sure to check formatting guidelines from AGU
\begin{table}
