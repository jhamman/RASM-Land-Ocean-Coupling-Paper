...
Approximately 11\% of the global terrestrial runoff drains into the Arctic Ocean, which holds only 1.4\% of the Earth's salt water \citep{Lewis_2000,Lammers_2001}.
...
Therefore, during the spring and summer months, the fractional contribution of freshwater to the Arctic Ocean from streamflow may be as high as 60\% (uncertainty in this figure is largely driven by the uncertainty in the seasonal cycle of the Bering Straight inflow) \citep{Serreze_2006}.
...
However, the earlier sea ice freeze-up enabled by low sea surface salinity (SSS) also reduces the amount of heat the upper ocean can lose in the, counteracting the impact freshening has on sea ice development \citep{Weatherly_1996,Morison_2012}.
...
The Coordinated Ocean-ice Reference Experiments Corrected Inter-Annual Forcing (CIAF) Version 2.0, hereafter referred to as $CORE.v2$, is a widely used ocean model forcing dataset that includes observed runoff at a selection of coastal rivers from \citet{Dai_2009}.
...
Most land surface components in Earth system models (ESMs), including the model used in RASM, do not include neighbor communication and therefore must utilize a separate routing scheme to transport streamflow across the land surface.
This process of transporting streamflow across the land surface is referred to as streamflow routing.
...
These CTC models have been shown to be difficult to parameterize across a range of spatial scales \citep{Sushama_2004}.
...
Those studies that have coupled streamflow routing with an ocean model have generally not been run at a sufficiently high spatial resolution required to resolve the coastal currents streamflow-shelf-basin exchange processes (e.g. eddies).
...
\citet{Dai_2009} extended a coastal subset of the R-ArcticNET database through 2007 in their study estimating the continental runoff flux.
\citet{Adam_2007}, \citet{Adam_2008}, \citet{Su_2005}, \citet{Slater_2007}, and \citet{Dai_2009} all have used uncoupled land surface models (LSMs), in conjunction with routing schemes to simulate streamflow across the pan-Arctic region.
...
Of particular relevance to this study, POP uses a virtual salinity flux ($VSF$) to represent changes in ocean salinity due to surface fluxes of freshwater (runoff, precipitation, and evaporation).
The $VSF$ is the equivalent amount of salt that would have to be added or removed to obtain the same change in salinity for a given fresh water flux.
It is calculated as
\begin{equation}
  \label{eq:SaltFlux}
  VSF=-F_w S
...
The ocean model was initialized from a no-motion state, with climatological temperature and salinity fields derived from the Polar Science Center Hydrographic Climatology version 3.0 \citep{Steele_2001}.
The model 75-year spin up consisted of an initial integration starting from 1948 through 1992 followed by a second integration from 1948 through 1979, both forced with $CORE.v2$.
...
In section \ref{sec:discussion}, we also compare the RASM coastal streamflow flux to $CORE.v2$ \citep{Large_2009} and the combined Greenland freshwater discharge climatology from $RACMO-GR$ \citep[e.g.][]{Bougamont_2005,Bougamont_2007,Rignot_2006}.
...
We also use data from the high-resolution (11 km) regional atmospheric climate model (RACMO2) applied over Greenland, hereafter referred to as $RACMO-GR$.
...
Figure \ref{fig:hydrographs} compares the monthly hydrographs for $RVIC_{FAST}$ and $RASM_{CONTROL}$ at the seven streamflow gauge locations shown in Figure \ref{fig:calibration_hydrographs}.
...
The peak spring freshet in $RVIC_{FAST}$ occurs one to two months early than $D2009$ and typically one month earlier than in $RASM_{CONTROL}$.
On average, this leads to overlap statistics in $RVIC_{FAST}$ that are about 15\% lower than for the $RASM_{CONTROL}$ simulation.
The differences in the routing parameters used in the $RVIC_{FAST}$ and $RASM_{CONTROL}$ and simulations can be clearly identified in the annual cycle column of Figure \ref{fig:hydrographs}.
The bias in the timing of the peak spring freshet is mostly due to the difference in streamflow velocity (2.0 vs. 0.6 m/s), whereas the shape of the hydrograph is largely determined by the diffusivity parameter (2,000 vs. 6,000 m\textsuperscript{2}/s).

The improvement in the $RASM_{CONTROL}$ statistics, relative to $RVIC_{FAST}$, highlights the impact of the parameter selection process and demonstrates the improvement that can be made through parameter optimization.
Furthermore, it also shows the limits of the calibration process method since the annual bias in volume is not under the control of the routing model.
...
\citet{Hamman_2016} provide a more detailed, intermodel comparison of the annual runoff biases in the Arctic and find that the performance of VIC in RASM is as good or better than other coupled models.
...
Biases in the winter baseflow flux have previously been identified VIC and other LSMs \citep{Slater_2007}.
...
Evidence for this can be found by comparing the timing of the peak runoff timing in Figures \ref{fig:calibration_hydrographs} and \ref{fig:hydrographs}.
Whereas the RASM simulations used for the calibration procedure had relatively small spring season temperature and snow melt timing biases \citep{Hamman_2016}, the RASM simulations here have introduced a premature snowmelt leading to minor timing biases in the peak annual streamflow.
The one exception to this explanation is in the Ob River basin, where the runoff regime is acutely influenced by extensive wetlands and permafrost, and the spring peak occurs a month early in both the calibrated and $RASM_{CONTROL}$ (May vs. June).

Figure \ref{fig:taylor} shows a Taylor Diagram comparing the RASM simulated hydrographs at 51 observation locations.
...
While the correlation coefficients in $RVIC_{FAST}$ are not arranged according to the annual flow volume, the largest basins in $RASM_{CONTROL}$ tend to perform better with correlation coefficients typically increasing by about 0.3.
...
Despite the relative contribution to the freshwater budget being larger in the Arctic than any other ocean body, the fraction of land represented by streamflow is substantially lower than the global average of 80\% \citep{Dai_2009}.
Given this data gap, models have often been used to estimate streamflow in ungauged regions.
Figure \ref{fig:coastal_hydrographs}, compares annual cycle of the RASM simulated coastal streamflow flux (boundaries shown in Figure \ref{fig:rasm_domain}) to the $CORE.v2$ and $D2009$ datasets.
In this figure, the $D2009$ data represents the total ``observed'' streamflow flux and has not been adjusted for the ungauged area whereas the $RASM_{CONTROL}$ and $CORE.v2$ datasets include fluxes gauged and ungauged areas.
The $D2009$ dataset is included as a lower limit on what the full coastal streamflow flux would be and provides a reference for the shape of the seasonal hydrograph.
...
Here again, the winter season baseflow flux bias is apparent in VIC, especially in the NW Canada and Alaska coast, and the Kara and Barents Sea masks.

% [Greenland: RASM vs. CORE II vs. sum of Dai/Trenberth vs. Bamber]
Runoff from Greenland (bottom of Figure \ref{fig:coastal_hydrographs}) is large contributor to the total coastal freshwater flux in the Arctic region, consisting of approximately 10\% of the total Arctic drainage area.
...
In Figure \ref{fig:coastal_hydrographs}, we again compare the coastal freshwater flux from Greenland to the $CORE.v2$ and $D2009$ datasets, as well as to the model estimates from $RACMO-GR$.
There are no observations on Greenland, and therefore $D2009$ is zero for all months.
...
Compared to $RACMO-GR$, $RASM_{CONTROL}$ has a similar annual average freshwater flux although the seasonal timing is out of phase by about one month.
It should be noted that the solid ice calving flux in $RACMO-GR$ is uniformly applied throughout the year, and that observational evidence \citep[e.g.][]{Joughin_2008} that indicates there is a seasonal cycle in this flux as well.
In any event, in terms of both the annual amplitude and mean, the $RASM_{CONTROL}$ represents a significant improvement, relative to $CORE.v2$ over greenland.

\subsection{Role of high-resolution coastal streamflow on ocean and sea ice dynamics}
\label{sec:ocean}

Figure \ref{fig:ocean_timeseries} shows the time series of streamflow for the Central Arctic and greater POP domain.
...
Figure~\ref{fig:ocean_timeseries} also shows the time series of SSS and sea surface temperature (SST) for the $RASM_{CONTROL}$ and $RASM_{NOROF}$ simulations.
...
In the Central Arctic, sea ice height for the $RASM_{NOROF}$ simulation is higher in all seasons by about 0.1 to more than 0.5 m, these differences are largest in the Laptev Sea.
...
\textit{Comment -- TODO: Determine if $RASM_{CONTROL}$ has an early or later freeze-up of sea ice, compared to $RASM_{NOROF}$.}
...
The RVIC model, coupled within RASM, effectively delivers streamflow to all coastal grid points draining to the POP ocean model.
We have also demonstrated that the parameters describing the linearized Saint-Venant equations are relatively easy to parameterize in RVIC through the use of a simple calibration procedure.
...
\citet{Lammers_2007} provided an estimate (0.2 $W/m^2$) of the heat flux derived from streamflow from the Russian portion of the Arctic basin.
...
\begin{itemize}[leftmargin=+.5in]
  ...
  \item Temporal resolution: the dataset includes mean daily streamflow fluxes between September 1, 1979 and December 31, 2014. Limited by the monthly availability of the observations in R-ArcticNET and $D2009$, $CORE.v2$ only included mean monthly streamflow fluxes. The higher temporal frequency of the streamflow in this dataset will better represent hydrologic extemes such as floods and low flow, and may be used to improved mesoscale process representation in ocean models (e.g. eddies, freshwater plumes).
...
This paper has introduced the RVIC model, demonstrated its ability to simulate the first-order routing processes in the Arctic, shown the relative importance of including the runoff flux in coupled ocean modeling applications, and provided a new dataset of spatially consistent high-resolution coastal streamflow fluxes for ocean modeling.
...
Of course, a more thorough parameter selection procedure can be used in which  watersheds would be calibrated individually using spatially-distributed velocity and diffusivity parameters derived directly from original sources (e.g. digital elevation models).
...
The dataset is provided at a daily timestep in netCDF4 file format for the dates between September 1, 1979, and December 31, 2014.
\end{itemize}

At the time of writing, the RASM model continues to be actively developed.
Planned improvements and extensions to this work include increased spatial resolution of the land, ocean, and runoff components (from 50 km to 10 km) as well as the ocean and sea ice components (from 9 km to 2 km).
...
Improvements to the representation of hydrologic processes in RASM (e.g. baseflow, snow melt, and permafrost dynamics) are also underway.
...
\caption{The Regional Arctic System Model domain, showing the 50-km near equal area domain shared by the land, atmosphere, and streamflow routing components (red/yellow shading), and the 1/12-degree ocean sea ice domain (blue shading). The RVIC drainage area is highlighted with gray shading.  The 7 largest river basins in the RASM domain are outlined in yellow: Amur (Am), Ob (Ob), Yenisey (Ye), Lena (Le), Mackenzie (Ma), Nelson (Ne), and Yukon (Yu).  The coastal streamflow flux masks used in Section \ref{sec:results} are outlined in green: Canadian Coast (Ca), Siberian Coast (Si), Kara and Barents Coast (KB), and Greenland (Gl). The location of the streamflow observations from $D2009$ are shown with black circles.}
...
Bottom: IRFs from the high-resolution (blue) and remapped (green) grids at the example point (61.62$^\circ$N, 121.16$^\circ$W) shown in A and B. The IRFs in C represent the streamflow response at the Mackenzie River upstream of the Arctic Red River for a unit impulse of runoff.}
...
\caption{Normalized hydrographs for largest 6 river basins from.  Each trace (grey) represents an individual calibration ensemble member. The observed (normalized) hydrograph for each basin is shown in blue and the hydrograph using the $Original$ or $Fast$ parameters is shown in dash-black.}
...
\caption{Taylor Diagram showing performance of the RVIC model ($RASM_{CONTROL}$ and $RVIC_{FAST}$) for 51 of the largest rivers in the RASM domain.}
\label{fig:taylor}
\end{figure}
