...
The $RASM_{CONTROL}$ hydrographs in the Amur, Lena, and Yukon basins match $D2009$ best, with normalized overlap statistics between 0.79 and 0.9.
...
The change in salinity during the first ten years of the $RASM_{CONTROL}$ simulation is mainly driven by a change in the streamflow flux (note the difference between $RASM_{CONTROL}$ and $RASM_{NOROF}$) but cannot be completely separated from.
...
The dataset described in this paper is publicly available via the University of Washington (\url{https://digital.lib.washington.edu/researchworks/}).
\end{acknowledgments}
