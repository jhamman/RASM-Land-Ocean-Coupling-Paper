...
As a result, the Arctic Ocean has the lowest salinity levels among the Earth's oceans \citep[e.g.][]{Steele_2001}.
Streamflow is the largest contributor of freshwater to the Arctic Ocean as it comprises approximately 38\% of the total freshwater flux entering the Arctic Ocean; the remainder of which consists of direct precipitation (24\%) over the Arctic Ocean, inflow from the Pacific Ocean (30\%), and inflow from the Atlantic Ocean (8\%) \citep{Serreze_2006}.
...
Coastal currents play important roles in shelf dynamics and shelf-basin interactions, redistributing both freshwater and heat through mixing \citep[e.g.][]{Carmack_1989,Rudels_1999,Ekwurzel_2001,Maslowski_2014}.
...
Thus, for a warming and freshening Arctic Ocean, the onset of freezing may partially buffer regional warming in areas highly influenced by streamflow.
However, the earlier sea ice freeze-up enabled by lower SSS also reduces the amount of heat the upper ocean can lose in the fall, potentially counteracting the impact freshening has on sea ice development \citep{Weatherly_1996,Morison_2012}.
...
This relatively strong pycnocline limits the heat flux into the surface mixed layer from the heat flux into the surface mixed layer from below.
...
\citet{Newton_2008} applied observed climatological runoff from nine of the largest rivers within the Arctic basin in the Naval Postgraduate School (NPS) Arctic coupled ice-ocean Model (NAME).
Their study used passive numerical flow tracers within their model to track the spatial distribution of runoff within the model domain.
They found the highest concentrations of river runoff along the Siberian coast and identified that freshwater plumes originating as coastal streamflow entered the central Arctic Ocean along topographic boundaries.
...
% [Paper overview]
We have implemented the RVIC streamflow routing scheme (named after the routing model typically used with the Variable Infiltration Capacity [VIC] hydrologic model) in order to represent the streamflow flux between the land and ocean model components within the recently developed Regional Arctic System Model (RASM) \citep{Roberts_2015a,DuVivier_2016,Hamman_2016}.
We introduce the new RVIC streamflow routing model in section \ref{sec:models}, its advancements in terms of parameterization of high-resolution streamflow routing, and its coupling within RASM.
...
In section \ref{sec:discussion} we compare two RASM simulations, one with coastal runoff and the other without, to highlight the role and importance of accurately representing streamflow in coupled climate simulations in the Arctic region.
Also discussed in this section is the production of a new spatially- and temporally-consistent, high-resolution dataset of coastal freshwater fluxes for the Arctic drainage basin and surrounding areas produced from a fully-coupled RASM simulation and intended for use in Arctic Ocean modeling applications.
...
The principal goals of this modeling project are: 1) to better understand the interaction between physical systems in the Arctic drainage basin, 2) to advance understanding of past and present states of Arctic climate, and 3) to improve seasonal to multi-decadal prediction capabilities of key Arctic indicators.
...
\item CICE: \citet{Roberts_2015a} provided a description of the coupling of version 4 of the Los Alamos Sea Ice Model (CICE) in RASM.
For this paper, we have upgraded  CICE in RASM to version 5 \citep{Hunke2015}, and incorporated the high-frequency sea ice coupling configuration of the developmental version of RASM described by \citet{Roberts_2015a}.
