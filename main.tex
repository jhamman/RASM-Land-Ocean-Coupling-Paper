...
New approaches to streamflow routing continue to be developed, adding new routing parameterizations and additional process representations such as reservoir operations, irrigation withdrawals, and overbank flow.
Recent examples include MOSART \citep{Li_2013}, CaMa-Flood \citep{Yamazaki_2009,Yamazaki_2014}, and mizuRoute \citep{Clark_2016}.
While a number of routing schemes have been coupled to ESMs \citep[e.g.][]{Olivera_2000,Sushama_2004}, they have generally been relatively simple models (no human management) and have not been extensively evaluated in terms of coupled land-ocean interactions.
...
RVIC, itself, is a relatively simple approach to streamflow routing and intended to provide a first order coupling of the land hydrology with the ocean component within RASM.
...
By calibrating to observed streamflows that inherently include human regulation activities such as dams, reservoirs, and withdrawals, we have built in a basic representation of these processes.
...
On the subject of reservoir operations, which arguably may be the most important processes not represented in our work, \citet{Adam_2007} provide a detailed analysis of the influence human management has on the annual hydrograph in the Lena, Yenisei, and Ob' Rivers.
Notably, that work showed fairly minor impacts on the annual hydrograph at the most downstream gauging locations, despite larger shifts in the hydrograph upstream.
Errors caused by not explicitly representing these processes are apparent, however, in some basins in the RASM domain, such as the Nelson River (bottom of Figure \ref{fig:hydrographs}), where RVIC produces a naturalized hydrograph that bears little resemblance to the observed hydrograph which is highly influenced by reservoir operations.
