...
As a result, the Arctic Ocean is contains the lowest salinity levels of any of Earth's oceans.
Streamflow is the largest contributor of freshwater to the Arctic Ocean and makes up approximately 38\% of the total freshwater freshwater flux entering the Arctic Ocean \citep{Serreze_2006}; the remainder of which largely consists of direct precipitation on the Arctic Ocean.
The streamflow contribution is strongly seasonality and during the spring and summer months, the fractional contribution of freshwater to the Arctic from streamflow is much larger than 38\%.
...
As \citet{Morison_2012} pointed out, earlier sea ice freeze-up enabled by low sea surface salinity (SSS) also reduces the amount of heat the upper ocean can loose in the fall, limiting the impact freshening has on sea ice development.
...
Although warmer water (less dense) exists at depth in the Arctic Ocean, stratification is maintained by the density differences between the cold fresh mixed layer and the more saline halocline and Atlantic water layers.
...
\citet{Newton_2008} applied observed climatological runoff from nine the largest nine rivers within the Arctic basin in the Naval Postgraduate School (NPS) Arctic regional Model (NAME).
...
They conclude that a proper representation of the streamflow flux is a key step toward improving the performance of the ocean model components in these models.

Numerous observational and modeling studies have explored the seasonal and inter-annual behavior of Arctic runoff.
\citet{Lammers_2001} compiled the R-ArcticNET database, a regional hydrographic record of mean monthly streamflow observations, including over 3,700 streamflow gauges in the Pan-Arctic region.
The collection of observations in R-ArcticNET was later used by \citet{Shiklomanov_2009} in their investigation of increasing river discharge in the largest Eurasian rivers and by \citet{Tan_2011} in their investigation of changes in spring snowmelt timing.
...
Source-to-sink routing methods do not explicitly track streamflow between grid cells.
Instead, they parameterize the distribution and travel time of runoff between source and outlet grid cells.
In previous applications of source-to-sink routing models within coupled climate models, such as \citet[][]{Olivera_2000}, the streamflow routing has been applied at course spatial resolutions (greater than 200 km) and low frequency coupling (e.g. daily).
Furthermore, these studies have typically focused on global applications and have not specifically evaluated the importance of streamflow in terms of coupled ocean interactions.
...
This notion is further echoed by \citet{Lique_2015} in their recent review paper on the representation of the Arctic hydrologic cycle in coupled climate models.
...
Our compares simulated streamflow to observations at streamflow gauging locations and explores the role of the terrestrial freshwater flux in the ice and ocean components within RASM.
Section \ref{sec:discussion} includes a discussion on the role of streamflow in the Arctic climate system the scientific advances and drawbacks associated with this research.
...
\begin{enumerate}
\item CICE: The Los Alamos Sea Ice model is physically based dynamic sea ice model \citep{Hunke_2010}.
...
The velocity and diffusivity parameters can be be estimated from observed streamflow, through numeric optimization, or by solving the Saint-Venant equations directly.
...
The flow along the travel path is parameterized as a linear time-invariant unit impulse response function (IRF) to discharge from the LSM, often referred to in the hydrology community as a unit hydrgraph (UH).
...
The main difference is in the implementation, where RVIC has moved the time integral to the outer loop.
This modification allows for stepwise evaluation of the convolution equation.
...
To simulate the dispersion of freshwater throughout each ocean grid cell within RASM, a diffusion scheme is applied within the coupler (CPL7) to avoid unrealistic salinity gradients that would occur when a river’s entire outflow is applied to one ocean grid cell.
The mapping from the runoff grid to the ocean grid is generated as a preprocessing step using the runoff and ocean grids and masks.
...
For this procedure, RVIC was run offline of RASM at a daily timestep and was forced using daily runoff and baseflow fluxes from the fully coupled RASM simulation described as $RASM_{ERA}$ in \citep{Hamman_2016}.
...
The pair of velocity and diffusivity parameters, 0.6 m/s and 3,000 m\textsuperscript{2}/s respectively, was chosen to maximize the volume weighted composite overlap statistic for the largest six rivers in the RASM domain (Figure \ref{fig:calibration_hydrographs}).
...
Both RASM simulations began with the same initial state (see \citet{Hamman_2016} for more details) and use identical land, atmosphere, ocean, and sea ice model configurations.
...
\begin{itemize}
     ...
     \item $RVIC_{FAST}$: stand-alone RVIC simulation forced with $RASM_{CONTROL}$.
          This simulation uses velocity and diffusivity parameters of 2.0 m/s and 2,000 m\textsuperscript{2}/s.
          These parameters are frequently used as default parameters with the RVIC model and were the original parameters used when developing RVIC within the coupled model.
\end{itemize}
...
This peak streamflow timing bias is likely due a spring and summer warm bias in the $RASM_{CONTROL}$ simulation \citep{Hamman_2016,Cassano_2016}, resulting in premature snow melt and runoff.
...
As is discussed is Section \ref{sec:hydrographs}, the winter-time streamflow minimum is likely underestimated in RASM due to cold season biases in the baseflow flux generated from VIC.
...
The SSS in $RASM_{NOROF}$ is in a transient state until about year 2000, while the $RASM_{CONTROL}$ reaches a steady state about 10 years earlier (c. 1990).
...
The differences in the SSTs between the two simulations are relatively small when averaged over the full ocean domain, $RASM_{CONTROL}$ simulation is found to be about 0.25 degrees C warmer than the $RASM_{NOROF}$ simulation in the central arctic basin.
...
While regions outside of central arctic are not significantly different between the two RASM simulations, the central Arctic basin is shown to be between 1 and 6 ppt fresher in $RASM_{CONTROL}$ than in $RASM_{NOROF}$.
...
This result partially corroborates the findings of \citep{Morison_2012} in so far as they also indicated, from an observational perspective, that a fresher arctic ocean would have less sea ice.
