...
RVIC is named after the routing model that has typically been used with the Variable Infiltration Capacity [VIC] hydrologic model % BN: include VIC reference.
...
Finally, in section \ref{sec:conclusions}, we provide our conclusions and highlight the potential for further research using RASM. % BN: Not a fan of forward looking conclusions as that work may never happen. Papers should be self-contained.
...
Model components are coupled using the Community Earth System Model (CESM) coupled model framework and the CPL7 flux coupler \citep{Craig_2011}. % BN: CESM mention needs reference
...
The virtual salinty flux is calculated as
\begin{equation}
  \label{eq:SaltFlux}
  VSF=-F_w S
\end{equation}
...
The 75-year ice-ocean spin-up consisted of an initial integration starting from 1948 through 1992 followed by a second integration from 1948 through 1979, both forced with $CORE.v2$ (see section \ref{sec:data}). % BN : I would collect initialization information for all model components in a single location as part of the Model Simulations section and move these two sentences to that section.
...
In their recent synthesis of CMIP5 runoff dynamics, \citet{Bring_2015} conclude that a significant community effort is required to improve the understanding and modeling of basin scale freshwater fluxes in coupled climate modeling. % BN : Explain acronyms when first used (CMIP5)
...
The IRF for every source-to-sink pair is a combination of an IRF that accounts for within grid cell flow processes and an IRF that accounts for the horizontal advection-diffusion between the edge of the grid cell and a downstream location.
...
Both RASM simulations began with the same initial state \citep[see ][]{Hamman_2016} and use identical land, atmosphere, ocean, and sea ice model configurations. % BN I would move the POP initialization discussion to here.
...
This dataset was further blended with model estimates to fill in ungauged areas and was adjusted to close the global water budget % BN : What is "This dataset". Is that CORE or D2009
and is frequently used as a forcing dataset for global and regional ocean modeling.
...
Because there are no observations on Greenland, the streamflow flux in $D2009$ is zero for all months.
...
% This would be a proper place to comment on the use of restoring to sea surface salinity climatology in ocean models, which does not provide spatial details and termporal variability, due to limited observations. WM comment
...
Beyond introducing the RVIC streamflow routing model, this paper also describes the associated coastal streamflow flux dataset which is made available.
...
This research was supported under United States Department of Energy (DOE) grants DE-FG02-07ER64460 and DE-SC0006856 to the University of Washington, and DE-SC0005783 and DE-SC0005522 to the Naval Post Graduate School.
...
The dataset described in this paper is publicly available via the University of Washington (https://digital.lib.washington.edu/researchworks). % BN : Is there a DOI for the data set?
\end{acknowledgments}
...
% BN : This comment was still in the document: FIX THIS CAPTION TO INDICATE THAT ONLY $RASM_{CONTROL}$ IS ONLY SHOWN INT THE TOP ROW, AND FIX ASSOCIATED DISCUSSION - SEE COMMENTS FROM WIESLAW
}
